package com.demo.news.agregator.demo.views.unian;
...
@PageTitle("Unian")
@Route(value = "unian", layout = MainLayout.class)
@AnonymousAllowed
public class UnianView extends Div implements AfterNavigationObserver {
    @Autowired
    ...
    private List<NewsDTO> fetchData() {
        ...
        return newsService.getAllNewsBySource(NewsSourceEnum.UNIAN_MAIN)
                .stream()
                .skip(offset)
                .limit(itemsPerPage)
    ...
    private void nextPage() {
        int totalItems = newsService.getAllNewsBySource(NewsSourceEnum.UNIAN_MAIN).size();
